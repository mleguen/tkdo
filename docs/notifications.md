...
For email notification troubleshooting (not receiving emails, too many emails, delays, etc.), see the [Troubleshooting Guide - Email Notification Issues](troubleshooting.md#email-notification-issues).
...
- Uses PHP's mail() function with system mail configuration
- Mailer settings in `api/.env` or environment variables
- From address: `TKDO_MAILER_FROM` (defaults to `Tkdo <noreply@{hostname}>`)
- SMTP configuration handled at system level (sendmail, msmtp, etc.)
...
Tkdo uses only one email-related environment variable:

- `TKDO_MAILER_FROM` - Sender email address (e.g., `Tkdo <noreply@example.com>`)
...
- Mail delivery errors are logged
...
- [Environment Variables Reference](environment-variables.md) - Email configuration variables
