...
- **Technical Tasks:**
  ...
  - Add comprehensive integration tests for notification API (NotifIntTest.php) covering instant notifications on idea creation and deletion, daily digest generation via notif-quotidienne.php command, notification preferences (None/Instant/Daily), email verification via Mailhog, notification filtering by occasion participants, author exclusion from notifications, digest includes both created and deleted ideas, coexistence of instant and daily notifications, digest not sent twice same day, digest not sent when no ideas present, and digest filtering by time period (11 tests, 17 with curl variants)
  - Add comprehensive integration tests for occasion draw API (OccasionIntTest.php) covering draw generation with various scenarios (simple draw, exclusions enforcement, impossible draw), draw regeneration with force parameter, past occasion validation, result visibility per participant ensuring users only see their own draw results, authentication and authorization checks for all draw operations (10 new tests, 20 with curl variants)
  - Add comprehensive API error response integration tests (ErrorHandlingIntTest.php) covering 400 Bad Request scenarios (invalid JSON, missing fields, unknown users, duplicate exclusions, already deleted ideas, already launched draws, already participating users), 401 Unauthorized responses (missing and invalid authentication tokens), 403 Forbidden responses (non-admin operations, non-author operations, non-participant access), 404 Not Found responses (unknown users, occasions, ideas), and validation error message consistency across all API endpoints (18 tests, 36 with curl variants)
  - Add comprehensive database constraint integration tests (DatabaseConstraintIntTest.php) verifying unique constraints (user identifiers, draw results per occasion), cascade deletion enforcement (prevent deletion of users/occasions with dependent records), and null constraints (required vs optional fields), with EntityManager reset handling for constraint violation testing (13 tests, 15 assertions)
