...
- [x] Task 6: Modify header component to show groups dropdown (AC: #1, #2, #3)
  ...
  - [x] 6.5 Group items render as disabled spans with "Bientôt disponible (Story 2.4)" tooltip — no navigation until Story 2.4 adds the route
...
- [x] [AI-Review][MEDIUM] Add test for malformed groupes$ API response to verify Array.isArray() fallback path [front/src/app/backend.service.spec.ts] — Add a test flushing `{ actifs: null, archives: null }` (or similar malformed shape) and asserting groupes$ emits `{ actifs: [], archives: [] }`. The map operator validation on backend.service.ts:152-155 has zero test coverage; the fallback branches are dead code from a testing standpoint.
- [x] [AI-Review][MEDIUM] Document and test estAdmin=false invariant for archived groups [api/test/Int/ListGroupeControllerTest.php:132-165 + api/src/Dom/Port/GroupePort.php] — JWT groupe_admin_ids only includes active-group admin IDs (readAppartenancesForUtilisateur filters archive=false), so archived groups always have estAdmin:false in the response. Add a test creating an archived group where the user is admin, verify estAdmin is false in body['archives']; add a code comment in GroupePort or encodeListeGroupes explaining why.
- [x] [AI-Review][LOW] Update Task 6.5 description to match implementation [_bmad-output/implementation-artifacts/2-3-view-my-groups.md:64] — Task 6.5 says "Group items link to /groupe/{id}" but implementation uses disabled <span> elements with no routerLink. Update the task description to: "Group items render as disabled spans with 'Bientôt disponible (Story 2.4)' tooltip — no navigation until Story 2.4 adds the route."
- [x] [AI-Review][LOW] Add @throws \RuntimeException docblock to GroupePort::listeGroupesUtilisateur [api/src/Dom/Port/GroupePort.php:26] — Method throws \RuntimeException (line 33) when the repository call fails, but the docblock only documents @return. Add: `@throws \RuntimeException if repository call fails`.
- [x] [AI-Review][LOW] Add estAdmin assertions to testListGroupeWithOnlyActiveGroups [api/test/Int/ListGroupeControllerTest.php:167-197] — Test creates 'Amis' (admin=true) and 'Famille' (admin=false) but never asserts their estAdmin values in the response. Add position-based assertions: assertTrue(body['actifs'][0]['estAdmin']) for 'Amis' (index 0, alphabetically first) and assertFalse(body['actifs'][1]['estAdmin']) for 'Famille'.
- [x] [AI-Review][MEDIUM] Replace name-keyed $adminFlags map with position-based assertions in testListGroupeIncludesEstAdminFlag [api/test/Int/ListGroupeControllerTest.php:158-164] — Uses `$adminFlags[$groupe['nom']] = $groupe['estAdmin']` (order-agnostic) unlike all other tests in the file (testListGroupeWithOnlyActiveGroups, testListGroupeArchivedGroupsAlwaysHaveEstAdminFalse) which use assertEquals by position. A sort regression in orderBy g.nom ASC would be invisible. Replace with: assertEquals('Admin Group', body['actifs'][0]['nom']), assertTrue(body['actifs'][0]['estAdmin']), assertEquals('Member Group', body['actifs'][1]['nom']), assertFalse(body['actifs'][1]['estAdmin']).
- [x] [AI-Review][LOW] Archived group sort assertion is database-collation-dependent [api/test/Int/ListGroupeControllerTest.php:109-110] — assertEquals('Été 2023', archives[0]['nom']) and assertEquals('Noël 2024', archives[1]['nom']) assume utf8mb4_unicode_ci (where 'É' sorts as 'E' → 'Été' < 'Noël'). Under utf8mb4_bin (byte order), 'É' (0xC3 0x89) > 'N' (0x4E), reversing the order and causing test failure on CI with binary collation. Either document the assumed collation in the test comment or replace 'Été 2023' with a name whose ASCII ordering is unambiguous (e.g., 'Automne 2023' so 'A' < 'N' regardless of collation).
- [x] [AI-Review][LOW] Add N+1 prevention comment to readToutesAppartenancesForUtilisateur [api/src/Appli/RepositoryAdaptor/GroupeRepositoryAdaptor.php:88] — Sibling method readAppartenancesForUtilisateur documents its ->addSelect('g') with "Eager-load Groupe entities to prevent N+1 queries when accessing getGroupe()->getId()". The new method uses the same pattern but has no such comment, risking future removal by a developer who thinks it's redundant.
- [x] [AI-Review][MEDIUM] Chain groupes$ from idUtilisateurConnecte$ instead of utilisateurConnecte$ to eliminate sequential HTTP waterfall [front/src/app/backend.service.ts:144] — Currently groupes$ waits for the user profile HTTP call to complete before starting the groups fetch. occasions$ (line 166) uses idUtilisateurConnecte$ and fires in parallel. Change to idUtilisateurConnecte$ and add 401/403 handling in catchError (matching occasions$ pattern) for faster time-to-content on page load.
- [x] [AI-Review][LOW] Replace name-keyed $adminFlags map with position-based assertions in testReadToutesAppartenancesForUtilisateurPreservesAdminFlag [api/test/Int/GroupeRepositoryTest.php:234-238] — Uses order-agnostic `$adminFlags[$a->getGroupe()->getNom()]` while sibling test (line 202-204) was updated to position-based assertEquals in review #5. Since readToutesAppartenancesForUtilisateur guarantees orderBy g.nom ASC and 'Groupe Admin' < 'Groupe Membre', use assertEquals on positions for consistency.
- [x] [AI-Review][LOW] Add tooltip text assertion to disabled group items test [front/src/app/header/header.component.spec.ts:181-192] — Test verifies disabled spans exist but never asserts title="Bientôt disponible (Story 2.4)". Add: expect(spans[0].getAttribute('title')).toBe('Bientôt disponible (Story 2.4)');
- [x] [AI-Review][MEDIUM] Add `#[\Override]` to `setUp()` in both new test files [api/test/Unit/Dom/Port/GroupePortTest.php:27, api/test/Unit/Appli/Service/JsonServiceGroupeTest.php:19] — Both files created in this story override PHPUnit's `TestCase::setUp()` without the mandatory `#[\Override]` attribute (project-context.md rule). `GroupeRepositoryTest.php:19` correctly has it; these two do not.
- [x] [AI-Review][MEDIUM] Set localStorage before asserting it's cleared in groupes$ 401 test [front/src/app/backend.service.spec.ts:322-335] — Test calls `service['idUtilisateurConnecte$'].next(1)` (not `localStorage.setItem()`), so `localStorage.getItem('id_utilisateur')` is already null from `beforeEach`'s `localStorage.clear()`. Assertion `toBeNull()` passes trivially without proving `effaceEtatLocal()` actually cleared anything. Add `localStorage.setItem(CLE_ID_UTILISATEUR, JSON.stringify(1))` before subscribing.
...
- ✅ Resolved review finding [MEDIUM]: Added frontend test for malformed `groupes$` API response — flushes `{ actifs: null, archives: null }` and asserts `Array.isArray()` fallback emits `{ actifs: [], archives: [] }`. Covers previously dead validation branches.
- ✅ Resolved review finding [MEDIUM]: Added `testListGroupeArchivedGroupsAlwaysHaveEstAdminFalse` integration test — creates archived group where user is admin in DB, verifies estAdmin=false in response (because JWT only carries active-group admin IDs). Added explanatory comment in `getPayloadGroupe()` documenting the invariant.
- ✅ Resolved review finding [LOW]: Updated Task 6.5 description to match implementation (disabled spans with tooltip, not routerLink).
- ✅ Resolved review finding [LOW]: Added `@throws \RuntimeException if repository call fails` to GroupePort docblock.
- ✅ Resolved review finding [LOW]: Added position-based estAdmin assertions to `testListGroupeWithOnlyActiveGroups` — `assertTrue` for 'Amis' (admin) and `assertFalse` for 'Famille' (member).

- ✅ Resolved review finding [MEDIUM]: Replaced name-keyed `$adminFlags` map with position-based `assertEquals` + `assertTrue`/`assertFalse` in `testListGroupeIncludesEstAdminFlag` to make sort regressions visible.
- ✅ Resolved review finding [LOW]: Replaced collation-dependent test name 'Été 2023' with ASCII-safe 'Automne 2023' ('A' < 'N' in all collations) in `testListGroupeReturnsActiveAndArchivedGroups`.
- ✅ Resolved review finding [LOW]: Added N+1 prevention comment to `readToutesAppartenancesForUtilisateur` matching sibling method's documentation pattern.
- ✅ Resolved review finding [MEDIUM]: Chained `groupes$` from `idUtilisateurConnecte$` instead of `utilisateurConnecte$` to eliminate sequential HTTP waterfall. Added 401/403 handling in catchError matching `occasions$` pattern. Groups fetch now fires in parallel with user profile fetch.
- ✅ Resolved review finding [LOW]: Replaced name-keyed `$adminFlags` map with position-based `assertEquals` in `testReadToutesAppartenancesForUtilisateurPreservesAdminFlag` — now asserts `appartenances[0]` = 'Groupe Admin' (admin=true), `appartenances[1]` = 'Groupe Membre' (admin=false) for sort regression visibility.
- ✅ Resolved review finding [LOW]: Added `title` attribute assertion to disabled group items test — verifies tooltip text 'Bientôt disponible (Story 2.4)'.
- ✅ Resolved review finding [MEDIUM]: Added `#[\Override]` attribute to `setUp()` in both `GroupePortTest.php` and `JsonServiceGroupeTest.php` — mandatory per project-context.md rule for all method overrides.
- ✅ Resolved review finding [MEDIUM]: Added `localStorage.setItem('id_utilisateur', JSON.stringify(1))` before subscribing in groupes$ 401 test — assertion now proves `effaceEtatLocal()` actually cleared the value (was previously trivially null from `beforeEach` clear).
...
- 2026-02-20: Sixth code review complete. 0 HIGH, 2 MEDIUM, 3 LOW issues. All require code/test changes — created 5 action items. Key findings: groupes$ map validation path untested (M1), estAdmin=false for archived groups undocumented and untested (M2), Task 6.5 description mismatch (L1), missing @throws docblock (L2), missing estAdmin assertions in testListGroupeWithOnlyActiveGroups (L3). Story status set to in-progress.
- 2026-02-20: Addressed sixth code review findings — 5 items resolved. Added malformed API response test for Array.isArray() fallback, added estAdmin=false archived-groups integration test with code comment, updated Task 6.5 description, added @throws docblock, added estAdmin assertions. 337 backend tests pass (1 new), 74 frontend tests pass (1 new). PHPStan level 8 clean.
- 2026-02-20: Seventh code review complete. 0 HIGH, 1 MEDIUM, 2 LOW issues. All require code/test changes — created 3 action items. Key findings: testListGroupeIncludesEstAdminFlag uses order-agnostic name map (M1), collation-dependent archived sort assertion (L1), missing N+1 comment in new repository method (L2). Story status set to in-progress.
- 2026-02-20: Addressed seventh code review findings — 3 items resolved. Position-based assertions in estAdmin test, ASCII-safe test names for collation independence, N+1 prevention comment. 337 backend tests pass, 74 frontend tests pass. PHPStan level 8 clean.
- 2026-02-21: Eighth code review complete. 0 HIGH, 1 MEDIUM, 2 LOW issues. All require code/test changes — created 3 action items. Key findings: groupes$ sequential HTTP waterfall via utilisateurConnecte$ chain (M1), inconsistent assertion pattern in repository admin flag test (L1), missing tooltip assertion in header spec (L2). Story status set to in-progress.
