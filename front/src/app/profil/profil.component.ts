...
export class ProfilComponent implements OnInit {
  ...
  async modifie() {
    ...
    try {
      ...
      for (const champ of ['mdp', 'confirmeMdp'])
        getChildControl(this.formProfil, champ).reset();
...
function getChildControl(group: AbstractControl, path: string) {
  ...
  if (!child)
    throw new Error(`le contrôle '${path}' n'existe pas dans le groupe`);
  return child;
}
...
function requireOne(...lists: string[][]): ValidatorFn {
  return (group) => {
    return lists.some((names) =>
      names.every(
        (name) => Validators.required(getChildControl(group, name)) === null,
      ),
    )
      ? null
      : { requireOne: true };
...
function sameValueIfDefined(name1: string, name2: string): ValidatorFn {
  return (group) => {
    ...
    return child1.value !== getChildControl(group, name2).value
      ? { sameValueIfDefined: [name1, name2] }
      : null;
  };
}
