...
- custom operation links do not appear in HAL response
  - api-platform HAL support for links is limited to GET links to subresources but does not consider operations,
    so we need to decorate the HAL ItemNormalizer to, once the normalization is done,
    iterate over operations allowed for the current normalization groups to add the corresponding links
    (reuse the same groups checking mechanims as already used by the ItemNormalizer to keep only allowed attributes for the groups)
